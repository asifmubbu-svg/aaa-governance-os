...
export async function renderAssistant(c){
  ...
  const aiSt = await DB.aiStatus();

  c.innerHTML=`
  <div class="page-head"><div><div class="eyebrow">Intelligence & Reporting</div><h1>Governance Smart Search</h1>
    <p>Ask questions in plain language. ${aiSt.configured?`Answers are generated by an AI model grounded strictly in your governance records (policies, risks, requirements, processes, DOA), with citations. Personal/employee data is not sent to the model.`:`Answers are retrieved from your live records with citations. Connect an AI key (GROQ_API_KEY) to enable model-generated answers.`}</p></div>
    <div class="page-actions"><span class="badge ${aiSt.configured?'b-green':'b-amber'}">${aiSt.configured?'AI model connected ('+(aiSt.model||'')+')':'Grounded search (no AI key)'}</span></div>
  </div>
  <div class="card pad">
    <div class="chat">
      ...
      ${aiSt.configured?'':`<div class="muted" style="font-size:11.5px;margin-top:8px">Running in grounded local mode (no AI key). Set GROQ_API_KEY on the server to enable model-generated answers.</div>`}
    </div>
  </div>`;

  const ctx={docs,domains,emps};
  const scroll=()=>{ const m=document.getElementById('msgs'); if(m) m.scrollTop=m.scrollHeight; };
  const send=async()=>{
    const q=document.getElementById('q').value.trim(); if(!q) return;
    document.getElementById('q').value='';
    history.push({role:'user',text:q});
    if(aiSt.configured){
      history.push({role:'ai',text:'<span class="muted">Thinking…</span>',pending:true});
      document.getElementById('msgs').innerHTML=history.map(renderMsg).join(''); scroll();
      try{
        const r=await DB.aiAsk(q);
        history.pop();
        if(r && r.answer){ history.push({role:'ai',text:aiHtml(r.answer),cites:(r.sources||[]).map(s=>({id:s.id}))}); }
        else { const a=answer(q,ctx); history.push({role:'ai',text:a.text,cites:a.cites}); }
      }catch(e){ history.pop(); const a=answer(q,ctx); history.push({role:'ai',text:'<span class="muted">(AI unavailable, showing grounded result)</span><br>'+a.text,cites:a.cites}); }
    } else {
...
// Render model output safely: escape HTML, linkify [RECORD-ID] citations, keep line breaks.
function aiHtml(text){
  let t = H.esc(text);
  t = t.replace(/\[([A-Z]{2,5}-[A-Za-z0-9-]+)\]/g, (m,id)=>`<span class="link" onclick="location.hash='#/repository/${id}'">[${id}]</span>`);
  return t.replace(/\n/g,'<br>');
}
